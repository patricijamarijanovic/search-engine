...
class Indexer:
    def __init__(
        ...
        self.documents_stats_path: str = os.path.join(
            self.output_directory, "documents_stats.jsonl"
        )
    ...
    def create_forward_index(self):
        ...
        gc.collect()
        print(
            f"Forward index created. Total documents: {document_id}, skipped: {skipped_documents}"
        )
    ...
    def _add_document(self, doc_id: int, tokens: list[str]):
        ...
        if (
            self.token_count >= self.token_threshold or mem_usage > memory_limit * 0.8
        ):  # if token threshold is passed, or memory limit, store index to disk
            self._write_block()
            self.token_count = 0
    ...
    def _finalize(self):
        ...
        meta_path = os.path.join(
            self.output_directory, "documents_metadata.jsonl"
        )  # storing documents metadata to the disk in jsonl format
